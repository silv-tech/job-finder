...
export default function ProfileSettings() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Imported Resume/Portfolio Content */}
      {profile.resume_text && (
        <div className="bg-white rounded-2xl p-6 border border-slate-200">
          <div className="flex items-center justify-between mb-3">
            <div>
              <label className="block text-sm font-medium text-slate-700">Imported Content</label>
              <p className="text-xs text-slate-400 mt-0.5">
                This is what the AI uses to write your applications. Everything here comes from your resume or portfolio.
              </p>
            </div>
            <button
              onClick={() => setProfile((p) => ({ ...p, resume_text: '' }))}
          ...
          <p className="text-xs text-slate-400 mt-2">
            {profile.resume_text.length.toLocaleString()} characters. You can edit this to add or remove details.
          </p>
        </div>
      )}
